...
class SyncEnginePushTest {
    ...
    @Test
    fun `accepted does not clobber a row re-edited while the push was in flight`() = runTest {
        db.transactionDao().upsert(pending(id = "t1", version = 0)) // localUpdatedAt = 1
        val api = FakePushApi(push = {
            // Simulate the user editing the same row while the request is in flight: the snapshot the
            // push reconciles against is now stale.
            kotlinx.coroutines.runBlocking {
                val current = db.transactionDao().getById("t1")!!
                db.transactionDao().upsert(
                    current.copy(
                        amount = 999,
                        sync = current.sync.copy(localUpdatedAt = 2L, syncStatus = SyncStatus.PENDING),
                    )
                )
            }
            SyncPushResponse(listOf(accepted("t1", serverVersion = 1)))
        })

        engine(api).push()

        val row = db.transactionDao().getById("t1")!!
        assertEquals(999L, row.amount) // the concurrent edit survived instead of being overwritten
        assertEquals(0, row.version) // base version unchanged → re-pushed on the next run
        assertEquals(SyncStatus.PENDING, row.sync.syncStatus) // still dirty
    }

    @Test
    fun `stale soft-delete is force-committed and pushed, a fresh one is left for undo`() = runTest {
        val old = System.currentTimeMillis() - 60_000 // past the grace window
        val fresh = System.currentTimeMillis()
        db.transactionDao().upsert(pendingDelete(id = "old", localUpdatedAt = old))
        db.transactionDao().upsert(pendingDelete(id = "fresh", localUpdatedAt = fresh))
        val api = FakePushApi(push = { SyncPushResponse(listOf(accepted("old", serverVersion = 3))) })

        engine(api).push()

        // Only the stale delete was promoted and pushed; the fresh one stays in its undo window.
        val pushedChange = api.requests.single().changes.single()
        assertEquals("delete", pushedChange.operation)
        assertEquals("old", pushedChange.data["id"]?.jsonPrimitive?.content)
        assertEquals(SyncStatus.PENDING_DELETE, db.transactionDao().getById("fresh")!!.sync.syncStatus)
    }
    ...
    /** A synced row soft-deleted in the undo window: hidden, but not yet pushable (pending_delete). */
    private fun pendingDelete(id: String, localUpdatedAt: Long) = TransactionEntity(
        id = id, name = null, type = "expense", amount = 500, currency = "JPY",
        date = "2026-06-02T14:30:00", memo = null, categoryId = "c1", accountId = null,
        isAutoGenerated = false, isBalanceAdjustment = false, recurringTransactionId = null,
        version = 2, createdAt = "2026-06-02T14:30:00", updatedAt = "2026-06-02T14:30:00",
        deletedAt = "2026-06-03T00:00:00",
        sync = SyncMeta(isSynced = false, syncStatus = SyncStatus.PENDING_DELETE, localUpdatedAt = localUpdatedAt),
    )
}
